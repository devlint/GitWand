...
- ✅ **Suggestions IA** : Composable `useAIProvider.ts` — abstraction multi-provider (Claude API, OpenAI-compatible, Ollama local) avec prompt système spécialisé résolution de conflits, parsing JSON structuré (resolvedContent/explanation/confidence). Panneau Settings restructuré en onglets (Général/Git/Éditeur/IA) — config dynamique par provider, détection auto Ollama, masquage API key, modèles pré-listés pour Claude. Bouton "IA" dans `MergeEditor.vue` sur chaque conflit hunk — appel asynchrone, pré-remplissage du mode édition avec la suggestion, bandeau d'explication, gestion d'erreur inline
