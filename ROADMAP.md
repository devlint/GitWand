...
- **Today Phase 3 — active mutations** — deferred remainder of the Today inbox (v3.0.0): real nudge / merge actions from the inbox cards, and a direct jump from "Resolve" into the conflict resolver (today routes to in-app PR review); leans on v3.8's undo safety net, since an inbox merge is itself the "more auto-apply" that v3.8 exists to make safe, and on this version's event-driven refresh so an action's result is reflected instantly instead of on the next poll
  - **Not in this version: forge-side auto-merge.** The inbox's dep-bump action performs an *immediate* merge and is gated by `mergeReadiness`, so it correctly reports "waiting: &lt;check&gt;" while CI runs. Queueing a merge for when checks pass needs a real per-forge call (`gh pr merge --auto`, GitLab's MWPS, Azure's auto-complete, Bitbucket has no equivalent). See v3.11.0.
