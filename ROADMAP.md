...
- **`packages/mcp` preview tests time out under parallel load** — `src/__tests__/preview.test.ts` spins real git repos and runs the predictor against them, at vitest's default 5s timeout. Two cases measured 5.3s and 6.8s when the whole monorepo suite ran in parallel, and pass in ~1s in isolation. Not a product bug, but it makes `pnpm -r run test` non-deterministic on a loaded machine, and CI runs the same way. Raise `testTimeout` for that file, or give the fixtures a warm cache.
