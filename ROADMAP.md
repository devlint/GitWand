...
_Ordered by priority (2026-07-03, renumbered 2026-07-08 after v3.4.0 shipped the conflict-engine bundle — token_level_merge, base recovery, the recoverable-before-model metric — instead of the planned secrets scanner; renumbered again 2026-07-10 after v3.5.0 shipped the secrets scanner and PR Review 2.0 together as a single combined release, closing the version gap for everything after it; renumbered again 2026-07-20 after v3.6.0 shipped a batch of smaller incremental items (post-checkout "Update branch" prompt, the #128 non-blocking rebase-conflict UX fix, CommitGraph animation polish) instead of the planned Commit Review chantier, pushing the whole thread down one slot; renumbered again 2026-08-19 after v3.6.6 shipped the dev-loop/CI-build-times chore — planned as v3.6.5, but that tag was consumed by an unplanned bugfix release — instead of resuming the thread; renumbered again 2026-08-19 after v3.7.0 shipped Commit Review, so the thread now resumes at v3.8). The thread: lay the safety net before shipping more auto-apply (v3.8), make the app reactive and fast (v3.9), close the resolution loop (v3.10), then workflow & comparison primitives (v3.11–v3.12), experimental voice input (v3.13), and the v4.0 code-intelligence headline._
...
- **Interactive rebase: re-surface split/edit affordances after a mid-rebase conflict handoff** — follow-up to the #128 non-blocking conflict handoff (`RebaseEditor` closes on a conflict halt so the rebase banner + inline `MergeEditor` take over): if the same rebase later hits an `edit`/`split` halt, only the banner's Continue/Skip/Abort are available — the "Split this commit…" affordance doesn't reappear because `RebaseEditor` is no longer mounted. Rare edge case, strictly better than the prior always-blocking modal.
