...
_v3.10.0 — Live Repo — shipped; see [Shipped](#shipped) below and the full lot-by-lot detail in [CHANGELOG.md](./CHANGELOG.md)._
...
- **libgit2 blame, deferred from v3.10.0** — `git_blame`'s fast path was written, tested and then left disabled. `blame_attribution_matches_the_cli_on_a_moved_block` exposed a git-version-dependent attribution divergence on a moved block between libgit2's bundled Myers xdiff and the CLI's histogram blame: green against git 2.50.1, red against 2.55.0. Blame attribution is user-visible, so the CLI stays authoritative. `libgit2_blame` and its tests are kept in tree as the regression guard; re-enabling it means resolving the divergence first, not deleting the test
...
- **Linux inotify validation, carried over from v3.10.0** — the watcher registers inotify watches across the working tree, and registration cost on a `node_modules`-sized checkout was never measured on real Linux hardware (the work was done on macOS, where FSEvents has a different cost model). Needs a measurement before the watcher is leaned on any harder, and before the v4.0 indexer becomes its second consumer
