...
- **The ten commands that can corrupt a working tree are now proven to refuse identically in both backends.** `pnpm dev:web` runs `dev-server.mjs`, not Rust, and the failure paths were where the two had already diverged unnoticed (`read_file` rejected non-UTF-8 in Rust and substituted `U+FFFD` in Node). `merge`, `cherry-pick`, `revert`, `reset`, `discard`, `checkout`, `switch-branch` and the three stash operations each get a parity test that asserts both sides refuse **and leave the same repository behind** — agreeing on "no" while leaving different working trees is not parity, because the state is what the user acts on next. The result: no divergence among the ten. That is the finding, and it is worth recording rather than leaving implicit.
